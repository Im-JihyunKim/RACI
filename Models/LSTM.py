...
class LSTM(nn.Module):
    def __init__(self, configs, input_dim, vm_x_dim_list, num_et,
        ...
        # self.adaptive_pool = nn.AdaptiveAvgPool1d(configs.emb_dim)
        ...
        # feature extractor
        self.backbone = nn.LSTM(configs.emb_dim, self.hidden_dim,
    ...
    def _init_hidden_state(self, batch_size, device):
        ...
        c_0 = Variable(torch.zeros(self.num_direction, batch_size, self.hidden_dim)).to(device)
        return h_0, c_0
    ...
    def forward(self, x):
        ...
        """
        h_0, c_0 = self._init_hidden_state(x.size(0), x.device)

        x = x.unsqueeze(-1).view(x.size(0), -1, self.n_et_vars)
        emb = self.et_emb_layer(x)                   # [B, emb_dim]
        z, _ = self.backbone(emb, (h_0, c_0))
        et_pred = self.et_predictor(z.mean(dim=1))
        return et_pred
    
    def predict_vm(self, x_list):
        h_0, c_0 = self._init_hidden_state(x_list[0].size(0), x_list[0].device)
